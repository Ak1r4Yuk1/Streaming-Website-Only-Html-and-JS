...
// ===== CARICAMENTO SEZIONE CONTINUA A GUARDARE (CORRETTO) =====
    async function loadContinuaDaCookie() {
        ...
        // Itera tutti i cookie
        for (let i = 0; i < allCookies.length; i++) {
            const cookie = allCookies[i].trim();
            const separatorIndex = cookie.indexOf('=');
            const name = cookie.substring(0, separatorIndex);
            
            // Cerca solo i cookie videoTime_
            if (!name.startsWith("videoTime_")) continue;

            const value = parseFloat(cookie.substring(separatorIndex + 1));
            // Ignora se il valore non è valido o se visto per meno di 10 secondi
            if (isNaN(value) || value < 10) continue; 

            hasItems = true;
            
            // Parsing: videoTime_type_id_Sxx_Exx
            ...
            try {
                ...
                if (type === 'tv' && parts.length >= 5) {
                    season = parts[3].replace('S', '');
                    episode = parts[4].replace('E', '');
                ...
                // Forza l'apertura dell'episodio specifico invece del selettore
                if (type === 'tv' && season && episode) {
                    card.addEventListener("click", async (e) => {
                        // FIX: Se l'utente ha cliccato il cestino (o una sua parte), FERMATI.
                        // Lascia che l'evento raggiunga il bottone rimuovi.
                        if (e.target.closest('.remove-btn')) return;

                        e.stopImmediatePropagation(); 
                        e.preventDefault();

                        // 1. Apri l'interfaccia player
                        await openPlayer(item);
                        
                        // 2. FORZA il caricamento del video specifico
                        console.log(`▶ Ripresa forzata serie: S${season} E${episode}`);
                        loadVideo(false, tmdbId, parseInt(season), parseInt(episode));
                    }, true); // true = fase di cattura (avviene prima del bubble)
                }
